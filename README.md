...
| **Helius** | Webhook indexer | `settle_match` events stream to `/api/webhooks/helius`; each event indexes player addresses, scores, asset, stake, and winner to power the on-chain leaderboard | [`src/pages/Leaderboard.tsx`](src/pages/Leaderboard.tsx) | [webhook config](https://dev.helius.xyz) |
